...
object  OX_Discriminater {
  ...
  def test(args: Array[String])={
    ...
    val Cutting_ans =
      formula.AnsCutter.cut(
        formula.AnsCutter.make_black_paper("testdate/"+fn)
      ).map(formula.AnsCutter.getbox(
        formula.AnsCutter.make_black_paper("testdate/"+fn),_)
      ).toArray
