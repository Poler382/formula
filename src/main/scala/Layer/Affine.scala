...
class Affine(val xn: Int, val yn: Int,
  val eps:T= 0.001f, val rho1:T = 0.9f, val rho2:T= 0.999f) extends Layer {
    ...
    W = (0 until W.size).map(_ => rand.nextGaussian.toFloat * 0.01f).toArray
    var b = Array.ofDim[Float](yn)
    var dW = Array.ofDim[Float](xn * yn)
    var db = Array.ofDim[Float](yn)
    ...
    override def forward(xs:Array[Array[Float]]):Array[Array[Float]] = {
      ...
      for(i <- 0 until yn;j <- 0 until xs.size){
        y(i*xs.size+j) = C(i*xs.size+j)+b(i)
      }
    ...
    override def backward(ds:Array[Array[Float]]):Array[Array[Float]] ={
      ...
      for(i <- 0 until yn;j <- 0 until ds.size) {
        db(i) += ds(j)(i)
      }
    ...
    def update_sgd() {
      ...
      b = b.zip(db).map{case (a,b) => a - ( b * lr ) }
    }
    def update() {
      dW = dW.map(_ / count)
      db = db.map(_ / count)
