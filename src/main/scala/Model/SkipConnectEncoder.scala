package Seq2Seq
class SkipConnectionEncoder(in: Int,hidden: Int,out: Int,layernum: Int=1){
  ...
  def forward(x: Array[Float])={
    var add = Embedding.forward(x)
    for(i <- 0 until layernum){
      var lstm = Drop_Unit(i).forward(LSTM_Unit(i).forward(add))
      add += lstm
    }
    pre_h = add
    add
  ...
  def backward(d: Array[Float])={
    ...
    for(i <- layernum-1 to 0 by -1){
      lstm_back += Drop_Unit(i).forward(LSTM_Unit(i).backward(lstm_back))
    }
