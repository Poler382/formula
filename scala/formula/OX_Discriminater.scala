  ...
  def select_Net(mode:String = "OX_defo",H:Int=90,W:Int=240) = {
    ...
    val g = mode match {
      ...
      case "OX_defo_detile" =>{
        ...
        val a = new Convolution_Matver(Fs,3,h,w,16,0.0005f)
        val b = new Pooling(2,16,h-Fs+1,w-Fs+1)
        val c = new ReLU()
        ...
        val a1 = new Convolution_Matver(Fs,16,h,w,32,0.0005f)
        val b1 = new Pooling(2,32,h-Fs+1,w-Fs+1)
        val c1 = new ReLU()
        ...
        w = (w-Fs+1)/2
        println(h,w)
        val a2 = new Convolution_Matver(Fs,32,h,w,64,0.0005f)
        val b2 = new Pooling(2,64,h-Fs+1,w-Fs+1)
        val c2 = new ReLU()
        ...
        w = (w-Fs+1)/2
        println(h,w)
        val a3 = new Convolution_Matver(Fs,64,h,w,128,0.0005f)
        val b3 = new Pooling(2,128,h-Fs+1,w-Fs+1)
        val c3 = new ReLU()
      ...
      case "OX_affine" =>{
        ...
        val a = new Affine(h*w*3,100)
        val b  = new ReLU()
        val c = new Affine(100,2)
        val sf= new SoftMax()
  ...
  def train(args: Array[String]): Unit = {
   ...
   // loads(network,biaspath)
    ...
    for(ep<- 0 until epoch){
      ...
      val (loss,count) = learning(network,Collect_ans,blackimg2,h,w,fn.size)
      ...
      var (testLOSS,testCOUNT) = forwarder(network,TCollect_ans,test_blackimg,h,w,fn_test.size)
      System.gc()
      ...
      if(ep % 100 == 0 || ep == epoch-1){
        saves(network,networkmode+timer.date+"#"+(count/fn.size).toString+"%")
        println("save bias")
      }
    ...
    savetxt_Float(train_LOSSLIST.full,"train_OX_LOSS_"+timer.date+"#"+train_ACCLIST.head.toString+"%","txt")
    savetxt_Float(train_ACCLIST.full,"train_OX_ACC_"+timer.date+"#"+train_ACCLIST.head.toString+"%","txt")
    savetxt_Float(test_LOSSLIST.full,"test_OX_LOSS_"+timer.date+"#"+train_ACCLIST.head.toString+"%","txt")
    savetxt_Float(test_ACCLIST.full,"test_OX_ACC_"+timer.date+"#"+train_ACCLIST.head.toString+"%","txt")

  }



  def learning(Net:List[Layer],target:Array[Float],imgSet:Array[Array[Float]],h:Int,w:Int,size:Int):(Float,Float)={
    var loss = Array.ofDim[Float](target.size,2)
    ...
    Range(0 ,y.size-1).map{i =>
      if(y(i).indexOf(y(i).max) == t(i).indexOf(t(i).max)) count += 1f
      loss(i) += (y(i) - t(i)).map(a => a*a)
    }

    backwards(Net.init,loss)

    updates(Net)

    (loss.map(_.sum).sum/size,count)
  }
  //test時に実行用
  def forwarder(Net:List[Layer],target:Array[Float],imgSet:Array[Array[Float]],h:Int,w:Int,size:Int):(Float,Float)={
    def flat_and_float(im: Array[Array[Array[Int]]]) = im.flatten.flatten.map(_.toFloat)
    var loss = 0f
    val yList = List[String]()
    var count = 0f
